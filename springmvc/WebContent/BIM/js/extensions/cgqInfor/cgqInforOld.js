...
function _Infor(viewer, options){
    ...
    this.label.innerHTML+='<div class="box" id="cgq_box" style="height: 100%"><div class="temBox"><label>当前设备：</label><span id="temName"></span></div>'+
...
_Infor.prototype.sensorInfo = function(){
    ...
    $('#show-sensor').unbind().click(function () {
        ...
        if ($("#ldInput").attr("model_id") == "" || $("#ldInput").attr("model_id") == undefined && window.hideList.length == 0) {
            alert("请选择楼栋");
        }else if ($("#ldInput").attr("model_id") != "" && !($('#lcInput').attr('lc_dbid'))){
            var thatModel=Object.values(JHCYW.models_obj[$("#ldInput").attr("model_id")-1])[0];
                ...
                if(item.length == 0){
                    ...
                    item.map(function(val,index){
                        ...
                        if (sensorName.indexOf('传感器')>=0 && sensorName.indexOf('[')>=0){
                            ...
                            sensorArr.push(arr);
                            sensorDbid.push(val);
                            sensorInfor.push({"name":sensorArr,"dbid":sensorDbid});
                            if(sensorinfoL == index){
                                $('#sensor_label').append('<label class="xtTitle" id="xtTitle">'+ $("#ldInput").val() +'传感器列表</label>');
                                $('#sensor_label').append('<div id="sensor-wrapper"></div>');
                                for(var i=0;i<sensorInfor.length;i++){
                                    var threeMesh = buildComponentMesh( self._viewer, sensorInfor[i].dbid[i] );
                                    ...
                                    dummyData.push({
                                        ...
                                        y:  sensorPos.y,
                                        z:  sensorPos.z,
                                        dbid:sensorInfor[0].dbid[i],
                                        name:arr[0]
                                    });
                                    $('#sensor-wrapper').append('<div class="sensorDiv"><span class="sensor" _name="' + sensorInfor[i].name[i][0]+ '" _id="_sensor" _dbid="'+ sensorInfor[i].dbid[i]+'">' + sensorInfor[i].name[i][0] + '</span><i class="layui-icon layui-icon-play iForMore" title="查看传感器更多数据"></i></div>');
                                }
            ...
            // 判断楼层按钮的传感器事件
        } else if (Number($('#lcInput').attr('lc_dbid')) != '' && $('#lcInput').val() != '') {
            //楼层传感器数据
            var thatModel=Object.values(JHCYW.models_obj[$("#ldInput").attr("model_id")-1])[0];
            var instanceTree = thatModel.getData().instanceTree;
            var sensorArr = [];
            ...
            if (sensorArr.length == 0 ) {
                ...
                $('#sensor_label').append('<span class="iSpan" id="_sensorCancel"><i class="fa fa-remove fa-lg"></i></span>');
                $('.sensor').unbind().click(function () {
                    var dbid=Number($(this).attr('_dbid'));
                    self._viewer.select([dbid]);
                    //传入传感器dbid
                    var threeMesh = buildComponentMesh( self._viewer, Number($(this).attr('_dbid')) );
                    var sensorPos = new Object();
                    sensorPos.x = (threeMesh.boundingBox.max.x+threeMesh.boundingBox.min.x)/2;
                    ...
                    self._viewer.loadExtension('Autodesk.FirstPerson', self._viewer.config)
                        .then(function () {
                            self._viewer.setActiveNavigationTool('firstperson'); //viewer3d需要开启第一视角工具
                            //self._viewer.impl.fitToView([dbid],thatModel);//?????
                            //重新设置相机的位置
                        });
                });
            }
            $('#_sensorCancel').unbind().click(function () {
                ...
                self._viewer.setActiveNavigationTool();
                document.getElementById('sensor_label').style.display = 'none';
                self._viewer.impl.fitToView(Number($("#lcInput").attr("lc_id")));
                $('#lcInput').removeAttr('lc_dbid');
                self._viewer.unloadExtension("markup3d");
            });
            //遍历传感器数组
...
_Infor.prototype.createBox = function(){
    ...
    label2.innerHTML+='<div class="box" id="cgq_box2" style="height: 100%"><div class="temBox"><label>当前设备：</label><span id="temName2"></span></div>'+
