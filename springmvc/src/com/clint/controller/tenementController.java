...
public class tenementController {
	...
	public void tenement(HttpServletRequest req, HttpServletResponse reponse, Model model) {
		...
		if (StringUtils.hasText(tenementname) && StringUtils.hasText(public_rental_record)) {
			addSql = "select t.limitarea,p.starttime,p.endtime from t_chooseroom_person p,t_bim_chooseroom_tenement t where t.phonenum='"
					+ tenementname + "' and t.public_rental_record='" + public_rental_record + "' and to_date('"
					+ current + "','yyyy-mm-dd hh24:mi:ss')>to_date(p.starttime,'yyyy-mm-dd hh24:mi:ss') and to_date('"
					+ current
					+ "','yyyy-mm-dd hh24:mi:ss')<to_date(p.endtime,'yyyy-mm-dd hh24:mi:ss') and p.tenementid=t.id";
		}
		...
		if (checkCookie.endsWith("true")) {
			if (jsonArray.size() > 0) {
				...
				req.getSession().setAttribute("startTime", obj.getString("STARTTIME"));
				req.getSession().setAttribute("endTime", obj.getString("ENDTIME"));
				rtns.put("success", submitVerify);
			} else {
				submitVerify = "用户未在当前选房时间段内！";
