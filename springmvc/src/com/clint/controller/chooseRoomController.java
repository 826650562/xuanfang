...
public class chooseRoomController {
	...
	public String chooseRoom(HttpServletRequest req, HttpServletResponse reponse, Model model) {
		...
		String addSql = "";
		String startTime = String.valueOf(req.getSession().getAttribute("startTime"));
		String endTime = String.valueOf(req.getSession().getAttribute("startTime"));
	...
	public String roomDetail(HttpServletRequest req, HttpServletResponse reponse, Model model) {
		...
		// 获取房屋状态
		String searchRoomString = "select c.status from t_bim_chooseroom_status s,t_bim_chooseroom_code c  where s.roomid='"
				+ roomId + "' and s.statusid=c.id and delete_tag='0'";
	...
	public String chooseRoomSuccess(HttpServletRequest req, HttpServletResponse reponse, Model model) {
		...
		if (StringUtils.hasText(build) && StringUtils.hasText(houseHold) && StringUtils.hasText(publicRentalRecord)) {
			...
			if (json.size() > 0) {
				...
				// 房屋状态入库
				addSql = "insert into t_bim_chooseroom_status (id, roomId, TENEMENTID, time, STATUSID,delete_tag) values ('"
						+ id + "', '" + roomId + "', '" + TENEMENTID + "', '" + time + "','" + STATUSID + "','0')";
		...
		if (!limitArea.equals("null")) {
			...
			try {
				...
				if (StringUtils.hasText(loudong)) {
					addSql = "select distinct t.loudong,t.id id, t.jz_floor floor, t.jz_household houseHold  from T_BIM_ROOM t ,t_bim_roomtype r  where t.loudong='"
							+ loudong
							+ "'and t.jz_housetype=r.roomtypename and  to_number(r.roominsidearea) between to_number("
		...
		if (!publicRentalRecord.equals("null")) {
			...
			if (StringUtils.hasText(publicRentalRecord)) {
				addSql = "select distinct t.PUBLIC_RENTAL_RECORD, t.id,t.tenementname name,t.phonenum tenementtel,t.limitarea,r.loudong,r.jz_floor floor,r.jz_household household,c.status,s.statusid,rt.roominsidearea area"
						...
						+ "'and t.id=s.tenementid and s.statusid=c.id and s.roomid=r.id and s.delete_tag='0' and (s.statusid='3' or s.statusid='1') and r.jz_housetype=rt.roomtypename";
				...
				if (json.size() <= 0) {
					addSql = "select distinct t.PUBLIC_RENTAL_RECORD,t.id,t.tenementname name,t.phonenum tenementtel,t.limitarea"
							+ "  from t_bim_chooseroom_tenement t " + "where t.PUBLIC_RENTAL_RECORD='"
							+ publicRentalRecord + "'";
				}
			}
			int deleteCount = this.mapService.countAll(
					"select count(*) from t_bim_chooseroom_status s,t_bim_chooseroom_tenement t where t.PUBLIC_RENTAL_RECORD='"
							+ publicRentalRecord + "' and t.id=s.tenementid and s.delete_tag='1'");
	...
	public void delChoose(HttpServletRequest req, HttpServletResponse reponse) throws IOException {
		...
		int count = this.mapService
				.countAll("select count(*)  from t_bim_chooseroom_status t where delete_tag='1' and t.tenementid='"
						+ tenementId + "'");
		JSONObject obj = new JSONObject();
		if (count >= 3) {
			obj.put("delCount", "已经退选三次!无法退选！");
		} else {
			String sql = "select * from t_bim_chooseroom_status where t.tenementid='" + tenementId
					+ "' and t.statusid='3'";
			...
			if (choosedList.size() > 0) {
				...
				delSql = "update t_bim_chooseroom_status t set delete_tag='1' where  t.tenementid='" + tenementId
						+ "' and t.delete_tag='0' and t.statusid='1'";
				this.mapService.execute(delSql);
				searchAql = "select distinct *  from t_bim_chooseroom_status t where t.tenementid='" + tenementId
						+ "' and t.delete_tag='0' and ( t.statusid='1' or  t.statusid='3')";
	...
	// 获取房屋列表
	/*
	 * @RequestMapping(value = "/showRoomStatus") public String
	 * showRoomStatus(HttpServletRequest req, HttpServletResponse reponse,Model
	 * model){ String addSql = ""; addSql =
	 * "select distinct l.ldh build,r.JZ_HOUSEHOLD household,r.JZ_FLOOR floor,r.id roomId from T_BIM_LDXX l,t_bim_room r where r.loudong=l.ldh and r.jz_housetype!='无'"
	 * ; List addList = this.mapService.getListBySql(addSql); JSONArray jsonArr
	 * = JSONArray.fromObject(addList);
	 * 
	 * model.addAttribute("roomAll",jsonArr); return "pages/showRoomStatus"; }
	 */
